...
from src.services.health_summary_service import get_health_summary_service
...
async def detect_posture_upload(
    ...
    if len(files) < 1:
        raise HTTPException(status_code=400, detail="需要至少上传1张图片进行坐姿检测")
...
async def stop_video_monitoring(
    ...
    video_service = get_video_stream_service()
    report = video_service.stop_session(current_user.id)
...
async def get_weekly_summary(
    target_date: Optional[str] = Query(None, description="目标日期，格式：YYYY-MM-DD，默认为今天"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_session)
):
    summary_service = get_health_summary_service()
    ...
    summary = await summary_service.get_weekly_summary(current_user.id, db, target_datetime)
...
async def get_all_weekly_summaries(
    limit: int = Query(10, ge=1, le=50, description="返回的记录数，默认10条，最多50条"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_session)
):
    summary_service = get_health_summary_service()
    
    summaries = await summary_service.get_all_weekly_summaries(current_user.id, db, limit)
...
async def generate_weekly_summary(
    target_date: Optional[str] = Form(None, description="目标日期，格式：YYYY-MM-DD，默认为今天"),
    regenerate_ai: bool = Form(False, description="是否重新生成AI分析"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_session)
):
    summary_service = get_health_summary_service()
    ...
    summary = await summary_service.generate_weekly_summary(current_user.id, db, target_datetime)
    
    if regenerate_ai or not summary.ai_analysis:
        await summary_service.generate_ai_analysis(summary, db)
        db.refresh(summary)
...
@router.post("/weekly/ai-analysis")
async def generate_ai_analysis_for_summary(
    summary_id: int = Form(..., description="周度汇总ID"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_session)
):
    from src.models.health import WeeklyHealthSummary
    
    summary = db.query(WeeklyHealthSummary).filter(
        WeeklyHealthSummary.id == summary_id,
        WeeklyHealthSummary.user_id == current_user.id
    ).first()
    ...
    summary_service = get_health_summary_service()
    result = await summary_service.generate_ai_analysis(summary, db)
